<?php
	...
	Class fieldMember extends Field{
		...
		function appendFormattedElement(&$wrapper, $data, $encode=false){
			...
			$wrapper->appendChild(new XMLElement($this->get('element_name'), NULL, array('username' => $data['username'], 'password' => $data['password'])));
		}
		...
		public function fetchMemberFromUsername($username){
			$ExtensionManager = new ExtensionManager($this->_Parent);
			$driver =& $ExtensionManager->create('members');
			$member_id = $this->Database->fetchVar('entry_id', 0, "SELECT `entry_id` FROM `tbl_entries_data_".$this->get('id')."` WHERE `username` = '".$username."' LIMIT 1");
