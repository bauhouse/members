<?php
	...
	Class extension_Members extends Extension {
		...
		public function __construct($args) {
			...
			**	3rd Party Integration to an existing Symphony account */
			...
			if(self::$debug) var_dump(get_class($this->Member));
		}
		...
		public function saveFbSettings($string){
			return @file_put_contents(MANIFEST . '/facebook-settings', $string);
		}
	...
	-------------------------------------------------------------------------*/
		...
		public function cbCheckFrontendPagePermissions($context) {
			...
			if(!$role->canAccessPage((int)$context['page_data']['id']) && !($this->Member instanceof FacebookMember && !$this->Member->isLoggedIn())) {
